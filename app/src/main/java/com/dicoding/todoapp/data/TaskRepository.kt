...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    // 4 : Use FilterUtils.getFilteredQuery to create filterable query
    // 5 : Build PagedList with configuration
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        val query = getFilteredQuery(filter)
        val pagedListConfig = PagedList.Config.Builder().setEnablePlaceholders(PLACEHOLDERS)
            .setPageSize(PAGE_SIZE).build()
        return LivePagedListBuilder(
            tasksDao.getTasks(query), pagedListConfig
        ).build()
    }
    ...
    fun getNearestStartTask(): Task {
        return tasksDao.getNearestStartTask()
    }

    fun getNearestEndTask(): Task {
        return tasksDao.getNearestEndTask()
    }
